...
/**
 * Tests:
 * 1. copyVerticalBits(): during pre-render (scanline -1) cycles 280-304 the vertical
 ...
 *    matches previous behaviour (sanity check that caching 15-fineX didn't break logic).
 */
public class PPUCopyVerticalAndFineXTapTest {
    ...
    private INesRom buildSimpleRom() {
        ...
        for(int r=0;r<8;r++) chr[r] = (byte)0xFF;
    ...
    ppu.forceCoreState(ppu.getMaskRegister(), ppu.getStatusRegister(), ppu.getCtrl(), -1, 279, initialV, t, 0, 0);
        // Advance to cycle 280 of pre-render so copyVerticalBits() region executes (280..304)
        while(!(ppu.getScanline()==-1 && ppu.getCycle()>=280)) {
            ppu.clock();
        }
        // After entering region, run a few cycles to ensure copy executed
        for(int i=0;i<10;i++) ppu.clock();
        int vAfter = ppu.getVramAddress() & 0x7BE0;
        int expected = t & 0x7BE0;
        assertEquals(expected, vAfter, String.format("Vertical copy mismatch expected=%04X got=%04X", expected, vAfter));
    }

    // Sanity: fineX tap caching did not shift boundary detection for fineX extremes 0 and 7.
    @Test
    public void testFineXTapCacheBoundaryExtremes() {
        ...
        for(int fineX=0; fineX<=7; fineX+=7) { // test 0 and 7
            ...
            write(emu, 0x2000, 0x00); write(emu, 0x2001, 0x01);
    ...
    private int boundary(int[] buf) {
        boolean seenOpaque=false; int y=0;
        for(int x=0;x<64;x++) {
            int v = buf[y*256+x] & 0x0F;
            if(!seenOpaque) { if(v!=0) seenOpaque=true; }
            else { if(v==0) return x; }
        }
        return -1;
    }
}
