 ...
 * - Provide simple connect() methods to inject concrete PPU/APU/Mapper/Controllers.
...
public class Bus implements iBus {
	...
	public void attachPPU(PPU ppu) { this.ppu = ppu; }
	public void attachAPU(APU apu) { this.apu = apu; }
	public void attachControllers(Controller p1, Controller p2) { this.pad1 = p1; this.pad2 = p2; }
	public void attachMapper(Mapper mapper, INesRom rom) { this.mapper = mapper; this.rom = rom; }
	...
	public int cpuRead(int address) {
		...
		if (address < 0x2000) {
			...
			// APU + IO (partial). For now, only controllers at $4016/$4017 are meaningful reads; others return 0.
			if (address == 0x4016) return pad1 != null ? (pad1.read() & 0xFF) : 0;
			if (address == 0x4017) return pad2 != null ? (pad2.read() & 0xFF) : 0; // APU frame counter read ignored for now.
			...
			if (mapper != null) return mapper.cpuRead(address); // allow mapper override
			return 0; // fallback none
		} else {
			// PRG ROM via mapper (or basic mirror logic)
			if (mapper != null) return mapper.cpuRead(address);
			// Fallback: open bus 0
			return 0;
		}
	}
	...
	public void cpuWrite(int address, int value) {
		...
		if (address < 0x2000) {
			...
			if (ppu != null) writePpuRegister(0x2000 + (address & 0x7), value);
			return;
		} else if (address < 0x4014) {
			// APU + IO writes
			if (address == 0x4016) { if (pad1 != null) pad1.write(value); if (pad2 != null) pad2.write(value); }
			// TODO: route APU register writes (0x4000-0x4013,4015,4017) once APU implemented.
			return;
		} else if (address == 0x4014) {
			...
			// PRG RAM via mapper
			if (mapper != null) mapper.cpuWrite(address, value);
			return;
		} else {
	...
	@Override
	public int read(int address) { return cpuRead(address); }
	@Override
	public void write(int address, int value) { cpuWrite(address, value); }

	/** Perform OAM DMA immediately (blocking copy of 256 bytes). */
	private void performOamDma() {
		if (pendingDmaPage < 0 || ppu == null) return;
	...
	public void clearRam() { for (int i = 0; i < ram.length; i++) ram[i] = 0; }
}
